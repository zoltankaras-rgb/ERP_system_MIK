...
def _generate_slots_for_day(
    ...
    if n <= 1:
        ...
        free = window_minutes - sum(holds)
        gap_minutes = max(0, min(30, int(free // n))) if free > 0 else 0

    assigned: List[Tuple[datetime, datetime]] = []
    result: Dict[str, Tuple[datetime, datetime, int]] = {}

    # Deterministické poradie (podľa hash)
    ...
    for it in ccp:
        ...
        if max_start < 0:
            # Fallback ak sa nezmestí: začiatok dňa
        ...
        ok = False
        # Skúsime nájsť miesto (50 pokusov)
        for i in range(50):
        ...
        if not ok:
            ...
            while t + timedelta(minutes=hold) <= end_dt:
                ...
                conflict = False
                for (as_, ae_) in assigned:
                    if _intervals_overlap(as_, ae_, cand_start, cand_end, 0): # 0 gap panic mode
                        conflict = True
                        break
                if not conflict:
                    result[bid] = (cand_start, cand_end, hold)
...
def _load_slots_for_batches(batch_ids: List[str]) -> Dict[str, Dict[str, Any]]:
    ...
    for i in range(0, len(batch_ids), 200):
        ...
        rows = db_connector.execute_query(
            f"""SELECT batch_id, slot_start, slot_end, hold_minutes 
                FROM haccp_core_temp_slots WHERE batch_id IN ({placeholders})""",
            tuple(ch)
        ) or []
        for r in rows:
...
def _auto_fill_measurements(ymd: str, day_items: List[Dict[str, Any]]) -> None:
    ...
    # Skontrolujeme, ktoré už majú dáta
    existing_bids = set()
    if batch_ids:
    ...
    for item in ccp_items:
        bid = item["batchId"]
        if bid in existing_bids: continue # Už má dáta, preskočiť
        ...
        # Generovanie teplôt
        rnd = random.Random(f"{bid}_temp_v2")
...
def list_items(days: int = 365):
    ...
    # Ak nájdeme vo výrobe produkt, ktorý nie je v defultoch, pridáme ho ako CCP!
    known_products = set(defaults.keys())
    production_products = set(_norm_name(r.get("productName")) for r in prod_rows if r.get("productName"))
    
    new_products = production_products - known_products
    for np in new_products:
        # Vložiť do DB ako CCP=1 (Varený)
        try:
            db_connector.execute_query(
            ...
            # Aktualizovať lokálny dict, aby sme to hneď videli
    ...
    for r in prod_rows:
        ...
        # Tu už by malo byť všetko True, lebo sme to auto-aktivovali
        is_req = d.get("isRequired", False)
        tl = d.get("targetLowC") if is_req else None
        th = d.get("targetHighC") if is_req else None
        hm = d.get("holdMinutes") if is_req else 10
        
        if is_req and tl is None: tl, th = 70.0, 71.9
        ...
        item = {
            ...
            "targetHighC": th,
            "holdMinutes": hm,
            "limitText": f"{tl}-{th}" if (tl and th) else "-"
        }
        items.append(item)
        by_date.setdefault(pd, []).append(item)

    # 5. Generovanie Slotov a Teplôt (Magia)
    for ymd, day_items in by_date.items():
        _ensure_slots_for_day(ymd, day_items)
        _auto_fill_measurements(ymd, day_items)

    # 6. Načítanie výsledkov z DB
    batch_ids = list(set(x["batchId"] for x in items))
    meas_map = {}
    
    if batch_ids:
        ph = ",".join(["%s"] * len(batch_ids))
        # Získame posledné meranie pre každý batch
        all_meas = db_connector.execute_query(
            f"""SELECT batch_id, measured_c, measured_at, measured_by, note, target_low_c, target_high_c, hold_minutes 
                FROM haccp_core_temp_measurements WHERE batch_id IN ({ph}) ORDER BY measured_at ASC""",
            tuple(batch_ids)
        ) or []
