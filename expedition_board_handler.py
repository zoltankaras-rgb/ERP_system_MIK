...
def get_b2b_special_notes():
    ...
    # Ak je piatok (4), chystá sa na pondelok (+3 dni)
    if dnes.weekday() == 4:
        cielovy_datum = dnes + timedelta(days=3)
    # Ak je sobota (5), chystá sa na pondelok (+2 dni)
    elif dnes.weekday() == 5:
        cielovy_datum = dnes + timedelta(days=2)
    # Nedeľa (6) až Štvrtok (3) -> chystá sa na zajtra (+1 deň)
    else:
    ...
    # 2. Vytiahneme len objednávky PRE CIEĽOVÝ DÁTUM
    sql = """
        SELECT 
    ...
    # Vrátime spojené dáta - aj dátum pre obrazovku, aj samotné poznámky
    return {
