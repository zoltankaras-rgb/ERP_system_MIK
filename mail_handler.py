...
import db_connector
from tasks import DEFAULT_FROM_EMAIL
...
# =================================================================
# === JEDNODUCHÝ PROGRAMOVÝ ODOSEL E-MAILU (PRE SCHEDULER / AI) ===
# =================================================================
# Jednoduchý helper pre interné odosielanie (scheduler, kalendár)
def send_email(to_addr: str, subject: str, body_text: str, html: str | None = None) -> None:
    """
    Interný helper – pošle e‑mail na jednu adresu.

    Používa rovnakú Flask-Mail konfiguráciu ako send_email_from_form,
    ale nepotrebuje Flask request ani prílohy.
    """
    if not to_addr:
        return

    try:
        from app import app, mail  # lazy import, aby sme sa vyhli cyklom
        from flask_mail import Message
    except Exception as e:
        print("[MAIL] Nepodarilo sa importovať app/mail:", e)
        return

    with app.app_context():
        try:
            msg = Message(subject=subject or "(bez predmetu)",
                          recipients=[to_addr])
            ...
            mail.send(msg)
            print(f"[MAIL] Email odoslaný na {to_addr} cez Flask-Mail helper.")
        except Exception as e:
            print(f"[MAIL] Nepodarilo sa odoslať email na {to_addr} v send_email(): {e}")
...
def posli_email(to_addr: str, subject: str, text: str) -> None:
    ...
    try:
        from mail_handler import send_email  # teraz už EXISTUJE
        send_email(to_addr, subject, text, html=None)
        return
    except Exception as e:
        print(f"[tasks] posli_email: mail_handler.send_email zlyhalo: {e}")
    ...
    try:
        ...
        with smtplib.SMTP("localhost") as s:
            s.send_message(msg)
        print(f"[tasks] posli_email: fallback SMTP -> {to_addr}")
    except Exception as e:
        # nechceme padnúť, len zalogujeme
        print(f"[tasks] posli_email: fallback SMTP zlyhal: {e}")
