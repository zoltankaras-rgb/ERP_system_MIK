...
from leader_handler import _table_cols
...
def get_production_profit_view(year, month):
    ...
    stock_rows = db_connector.execute_query("""
        SELECT ean, COALESCE(aktualny_sklad_finalny_kg, 0) AS exp_stock_kg
        FROM produkty
        WHERE ean IS NOT NULL AND ean <> ''
    """) or []
    exp_stock_map = {
        str(r["ean"]): float(r.get("exp_stock_kg") or 0.0)
        for r in stock_rows
    }
    ...
    for p in all_products:
        ...
        manual_data    = prod_manual_data.get(ean, {}) or {}
        prod_cost      = float(prod_costs.get(p['nazov_vyrobku'], 0) or 0)
        # TU opravujeme default Príjem Exp. na +20 %
        transfer_price = float(
            manual_data.get('transfer_price_per_unit')
            or (prod_cost * 1.2 if prod_cost > 0 else 0)
        )
        sales_kg       = float(manual_data.get('expedition_sales_kg') or 0)
        exp_stock_kg   = float(exp_stock_map.get(ean, 0.0))
        ...
        _table_cols.append({
            ...
            "exp_stock_kg": exp_stock_kg,           # ← sem ide sklad
    ...
    return {"rows": _table_cols, "summary": summary}
...
def save_production_profit_data(data):
    ...
    data_to_save = [
        (
            ...
            month,
            row['ean'],
            float(row.get('expedition_sales_kg') or 0),
            float(row.get('transfer_price') or 0)
        )
        for row in rows
    ]
