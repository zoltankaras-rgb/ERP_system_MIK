...
(function (root, doc) {
  ...
  async function loadCustomersAndPricelists() {
    ...
    try {
        ...
        state.customers = data.customers || []; state.pricelists = data.pricelists || []; state.mapping = data.mapping || {};
        ...
        state.customers.forEach(c => {
            ...
            html += `<tr><td>${escapeHtml(c.zakaznik_id)}</td><td><strong>${escapeHtml(c.nazov_firmy)}</strong></td><td>${escapeHtml(c.email)}<br>${escapeHtml(c.telefon)}</td><td>${assignedNames || '<span class="muted">Žiadne</span>'}</td><td><button class="btn btn-primary btn-sm" onclick="window.editB2BCustomer(${c.id})">Upraviť</button></td></tr>`;
        });
  ...
  window.editB2BCustomer = function(id) {
      ...
      openModal(`<div style="display:grid; grid-template-columns: 1fr 1fr; gap:20px;"><div><h4>Fakturačné údaje</h4><div class="form-group"><label>ID</label><input type="text" value="${escapeHtml(cust.zakaznik_id)}" disabled class="filter-input" style="width:100%; background:#eee;"></div><div class="form-group"><label>Firma</label><input type="text" id="ced-name" value="${escapeHtml(cust.nazov_firmy)}" class="filter-input" style="width:100%;"></div><div class="form-group"><label>Email</label><input type="text" id="ced-email" value="${escapeHtml(cust.email)}" class="filter-input" style="width:100%;"></div><div class="form-group"><label>Telefón</label><input type="text" id="ced-phone" value="${escapeHtml(cust.telefon)}" class="filter-input" style="width:100%;"></div><div class="form-group"><label>Adresa</label><textarea id="ced-addr" class="filter-input" style="width:100%;">${escapeHtml(cust.adresa)}</textarea></div></div><div><h4>Priradené cenníky</h4><div style="max-height:300px; overflow-y:auto; border:1px solid #ddd; padding:10px; border-radius:4px;">${plHtml}</div><h4 style="margin-top:20px;">Iné</h4><label><input type="checkbox" id="ced-active" ${cust.je_schvaleny ? 'checked' : ''}> Účet je aktívny</label></div></div><div style="margin-top:20px; text-align:right;"><button class="btn btn-success" onclick="window.saveB2BCustomer(${cust.id})">Uložiť zmeny</button></div>`);
  };

  window.saveB2BCustomer = async function(id) {
      const payload = {
          id: id, nazov_firmy: doc.getElementById('ced-name').value, email: doc.getElementById('ced-email').value, telefon: doc.getElementById('ced-phone').value, adresa: doc.getElementById('ced-addr').value,
          je_schvaleny: doc.getElementById('ced-active').checked ? 1 : 0, pricelist_ids: Array.from(doc.querySelectorAll('.pl-check:checked')).map(cb => cb.value)
      };
      try {
          await callFirstOk([{ url: '/api/kancelaria/b2b/updateCustomer', opts: { method: 'POST', body: payload } }]);
          showStatus('Zákazník uložený'); closeModal(); loadCustomersAndPricelists();
      } catch(e) { alert(e.message); }
  };
  ...
  async function loadPricelistsForManagement() {
    ...
    try {
        ...
        // Handler pre VYMAZANIE
        box.querySelectorAll('button[data-del-pl]').forEach(b => {
            b.onclick = async () => {
                if (!confirm("Naozaj chcete vymazať tento cenník? Táto akcia je nevratná a odstráni cenník aj všetkým priradeným zákazníkom.")) return;
                
                try {
                    await callFirstOk([{ 
                        url: '/api/kancelaria/b2b/deletePricelist', 
                        opts: { method: 'POST', body: { id: b.dataset.delPl } } 
                    }]);
                    showStatus('Cenník bol vymazaný.', false);
                    loadPricelistsForManagement(); // Obnoviť zoznam
                } catch(e) {
  ...
  function showPricelistEditor(plId) {
      ...
      area.innerHTML = `
        <div class="stat-card" style="margin-top:20px; border:2px solid #e2e8f0;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px; border-bottom:1px solid #eee; padding-bottom:10px;"><h3 style="margin:0;">${isEdit ? 'Úprava cenníka' : 'Nový cenník'}</h3><button class="btn btn-secondary btn-sm" onclick="document.getElementById('pl-editor-area').innerHTML=''">Zavrieť</button></div>
            <div class="form-group"><label>Názov cenníka</label><input type="text" id="pl-name" class="filter-input" style="width:100%; font-size:1.1rem;"></div>
            ...
            ${isEdit ? `<div style="display:grid; grid-template-columns: 1fr 1fr; gap:20px;"><div><label>Katalóg produktov (Filter)</label><input type="text" id="pl-prod-filter" class="filter-input" style="width:100%; margin-bottom:5px;" placeholder="Hľadať..."><div id="pl-source-list" class="cust-select-container" style="height:400px;"></div></div><div><label>Položky v cenníku</label><div id="pl-target-list" class="cust-select-container" style="height:400px;"></div></div></div>` : ''}
      ...
      if (isEdit) {
          ...
          doc.getElementById('pl-prod-filter').addEventListener('input', (e) => renderSourceProducts(e.target.value));
      } else {
          doc.getElementById('pl-save-btn').onclick = async () => {
              ...
              try { await callFirstOk([{ url: '/api/kancelaria/b2b/createPricelist', opts: { method: 'POST', body: { name, customer_ids: selectedCusts } } }]); showStatus('Cenník vytvorený'); loadPricelistsForManagement(); } catch(e) { showStatus(e.message, true); }
          };
      }
  }
  ...
  function renderSourceProducts(filter) {
      ...
      state.productsAll.forEach(p => {
          ...
          if (!f || p.nazov_vyrobku.toLowerCase().includes(f) || p.ean.includes(f)) {
              html += `<div class="cust-option" style="justify-content:space-between;"><div><div style="font-size:0.9rem; font-weight:600;">${escapeHtml(p.nazov_vyrobku)}</div><small>${p.ean}</small></div><div style="display:flex;gap:5px;"><input type="number" id="price-in-${p.ean}" placeholder="Cena" style="width:70px; padding:2px;" step="0.01"><button class="btn btn-primary btn-sm" onclick="window.plAdd('${p.ean}')">+</button></div></div>`;
              count++;
          }
      });
