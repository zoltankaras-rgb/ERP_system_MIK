...
(async function () {
  ...
  if (typeof window.apiRequest !== 'function') {
    window.apiRequest = async (url, options = {}) => {
      ...
      if (!r.ok) {
        return {
          error: (payload && payload.error) || r.statusText || `HTTP ${r.status}`,
          status: r.status,
          raw: payload
        };
      }
  ...
  function iconForFolder(f) {
    return f === 'INBOX' ? 'fa-inbox'
         : f === 'SENT'  ? 'fa-paper-plane'
         : f === 'SPAM'  ? 'fa-ban'
         : f === 'TRASH' ? 'fa-trash'
         : 'fa-box-archive';
  }
  ...
  function setView(mode) {
    ...
    // keď nie sme v compose/signatures, zruš TinyMCE
  ...
  window.initializeMailModule = function initializeMailModule() {
          ...
          <div id="list-view" class="view active">
            <div class="card">
              <table id="mail-table" class="table">
                ...
                <tbody>
                  <tr><td colspan="4" style="padding:1rem;color:#6b7280;">Načítavam…</td></tr>
                </tbody>
              </table>
    ...
    async function loadSummary() {
        ...
        if (ul) {
          ...
          if (!top.length) {
            ...
            top.forEach(c => {
              ...
              li.innerHTML = `
                <a href="#" class="customer-link" data-customer="${c.customer_id}" style="display:flex; gap:.5rem;">
                  <i class="fas fa-user-tag"></i>
                  <span>${escapeHtml(c.customer_name || ('ID ' + c.customer_id))}</span>
            ...
            ul.querySelectorAll('.customer-link').forEach(a => {
              a.onclick = (ev) => {
                ev.preventDefault();
                state.customerId = a.dataset.customer || '';
                state.page = 1;
                setView('list');
                loadList();
              };
      ...
      if (cl && !cl.error && sel) {
        const cur = sel.value;
        sel.innerHTML = '<option value="">— zákazník —</option>' +
          (cl.items || []).map(c =>
            `<option value="${c.customer_id}">${escapeHtml(c.customer_name || ('ID ' + c.customer_id))}</option>`
          ).join('');
    ...
    function setLoadingList(on) {
      if (on) {
        elTbody.innerHTML = `<tr><td colspan="4" style="padding:1rem;color:#6b7280;">Načítavam…</td></tr>`;
      }
    }
    ...
    async function loadDetail(id) {
      ...
      const readerActions = container.querySelector('#reader-actions');
      const readerBody    = container.querySelector('#reader-body');

      readerActions.innerHTML = `
        <div style="display:flex; gap:.5rem; flex-wrap:wrap;">
          <button class="btn-primary"   id="btn-reply"><i class="fas fa-reply"></i> Odpovedať</button>
          <button class="btn-secondary" id="btn-archive"><i class="fas fa-box-archive"></i> Archív</button>
          <button class="btn-secondary" id="btn-spam"><i class="fas fa-ban"></i> Spam</button>
      ...
      await apiRequest(`/api/mail/messages/${id}/mark_read`, {
        method: 'POST',
        body: { read: true }
      });
      ...
      readerActions.querySelector('#btn-delete').onclick = async () => {
        await apiRequest(`/api/mail/messages/${id}`, { method: 'DELETE' });
        showStatus('Presunuté do Koša.');
        closeToList();
        loadList();
      };

      readerActions.querySelector('#btn-spam').onclick = async () => {
        await apiRequest(`/api/mail/messages/${id}/move`, {
          method: 'POST',
          body: { folder: 'SPAM' }
        });
        showStatus('Presunuté do SPAM.');
        closeToList();
        loadList();
      };

      readerActions.querySelector('#btn-archive').onclick = async () => {
        await apiRequest(`/api/mail/messages/${id}/move`, {
          method: 'POST',
          body: { folder: 'ARCHIVE' }
        });
        showStatus('Presunuté do ARCHÍVU.');
        closeToList();
        loadList();
      };

      readerActions.querySelector('#btn-reply').onclick = () => {
        openComposeView(`Re: ${d.subject || ''}`, d.from_email, d);
      };

      // zákazníci – naplnenie selectu + priradenie + mapovanie domény
      const cl  = await apiRequest('/api/mail/customers');
      ...
      readerBody.querySelector('#btn-assign').onclick = async () => {
        if (!sel) return;
        const cid = sel.value;
        if (!cid) return showStatus('Najprv vyber zákazníka.', true);

        const rr = await apiRequest(`/api/mail/messages/${id}/assign_customer`, {
          method: 'POST',
          body: { customer_id: cid }
        });
        if (rr && rr.error) return showStatus(rr.error, true);
        showStatus('E-mail bol priradený k zákazníkovi.');
        loadSummary();
      };

      readerBody.querySelector('#btn-map-domain').onclick = async () => {
        if (!sel) return;
        const fe = (d.from_email || '').trim();
        if (!fe || !fe.includes('@')) return showStatus('Neznámy odosielateľ.', true);
        const dom  = fe.split('@').pop();
        const cid  = sel.value;
        if (!cid) return showStatus('Vyber zákazníka v selecte.', true);
        const name = sel.options[sel.selectedIndex].textContent;

        const rr = await apiRequest('/api/mail/contact_links', {
          method: 'POST',
          body: {
            email: fe,
            customer_id: cid,
        ...
        if (rr && rr.error) return showStatus(rr.error, true);
        showStatus('Mapovanie domény bolo uložené.');
        loadSummary();
      };
    }
    ...
    async function openComposeView(subjectPrefill = '', toPrefill = '', replied = null) {
      ...
      const signatureOptions = (sigs.items || []).map(s => `
        <option value="${s.id}" ${def.item && def.item.id === s.id ? 'selected' : ''}>
          ${escapeHtml(s.name)}
        </option>`).join('');
    ...
    async function openSignaturesManager() {
      ...
      wrap.querySelectorAll('.btn-set-default').forEach(btn => {
        btn.onclick = async () => {
          ...
          const res = await apiRequest(`/api/mail/signatures/${id}`, {
            method: 'PUT',
            body: { name, is_default: true }
          });
          if (res && res.error) {
            showStatus('Nastavenie defaultu zlyhalo. Otvor editor a ulož s „default“.', true);
            return;
          }
          showStatus('Nastavené ako default.');
          openSignaturesManager();
        };
      });
      ...
      function showSigEditor(id, name, html) {
        ...
        try {
          if (window.tinymce && tinymce.get('sig-html')) tinymce.remove('#sig-html');
        } catch (_) {}
        ...
        document.getElementById('sig-cancel').onclick = () => {
          ...
          try { if (window.tinymce) tinymce.remove('#sig-html'); } catch (_) {}
        };

        document.getElementById('sig-save').onclick = async () => {
          const nm = document.getElementById('sig-name').value.trim();
          const ht = (window.tinymce && tinymce.get('sig-html'))
            ? tinymce.get('sig-html').getContent()
            : document.getElementById('sig-html').value;

          if (!nm) return showStatus('Zadaj názov podpisu.', true);

          if (id) {
            await apiRequest(`/api/mail/signatures/${id}`, {
              method: 'PUT',
              body: { name: nm, html: ht }
            });
          } else {
            await apiRequest('/api/mail/signatures', {
              method: 'POST',
              body: { name: nm, html: ht }
            });
          }
    ...
    container.querySelector('#btn-sync').onclick       = async () => {
      const r = await apiRequest('/api/mail/imap/fetch?limit=50', { method: 'POST' });
      if (r && r.error) return showStatus('Sync zlyhal: ' + r.error, true);
      showStatus(`Načítaných ${r.fetched || 0}, preskočených ${r.skipped || 0}.`);
      loadList();
    };
    container.querySelector('#btn-imap-test').onclick  = async () => {
      const r = await apiRequest('/api/mail/imap/test');
      showStatus(r && r.message ? r.message : 'IMAP OK');
    };
    ...
    container.querySelector('#btn-search').onclick = () => {
      state.query      = container.querySelector('#mail-search').value.trim();
      state.page       = 1;
      state.customerId = '';
      loadList();
    };
    ...
    container.querySelector('#btn-clear').onclick = () => {
      state.query      = '';
      state.customerId = '';
      container.querySelector('#mail-search').value          = '';
      container.querySelector('#mail-filter-customer').value = '';
      state.page = 1;
      loadList();
    };

    container.querySelector('#mail-filter-customer').onchange = (e) => {
